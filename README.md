...
| MLA-style latent KV compression (serve-time) | KV `[B,H,S,D]`→`[B,H,S,latent]` (memory ∝ latent/D); reconstruction error on read = quality cost | Calibrated PCA basis (`run_kv_compression_quality.py`) or random orthonormal fallback (POOR quality — warn at init). Validated on transformers 5.14.1: latent≥64 (D=128) reproduces greedy output token-for-token; too-small latent diverges (quality cliff). Gated by `KV_COMPRESSION=mla`; mutually exclusive with KV eviction AND prefix reuse. Basis loaded/validated at engine init |
...
- **Prefix KV reuse** — `PREFIX_CACHE_ENABLED=1` skips re-prefilling shared prompt prefixes. On the HF engine this resumes via a forward (`model(suffix, past_key_values=clone(cached))`) + manual decode loop — transformers ≥5 `model.generate` cannot resume from a pre-filled `past_key_values` (it drops the external input). vLLM uses native `enable_prefix_caching`. Incompatible with KV eviction and MLA compression.
- **MLA latent KV compression** — `KV_COMPRESSION=mla` stores a low-dim latent per token and reconstructs on read; set `KV_COMPRESSION_LATENT` (choose ≥ ~half the head dim; too small diverges) and optionally `KV_COMPRESSION_BASIS` (`.pt` calibration; without it a random projection is used and a warning is logged). Basis is loaded and shape-validated at engine init. Incompatible with KV eviction and prefix reuse.
