...
| Completed-prefix control | Engine-owner updates on A5000/A4000 protect matching completed-request blocks, clear protection on Resume and reject stale ownership/sequence; recycled blocks are reported absent. Authenticated control ingress has CPU contract coverage; GPU transport and Harness integration remain pending |
| Cache control ingress | Authenticated Gateway → Scheduler owner lookup → Worker journal/engine binding is implemented and CPU-tested; no reservation mutations. This transport has not yet passed a GPU run |
