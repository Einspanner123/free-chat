...
后续新增功能和缺陷修复均按下列顺序执行，不先写实现再补测试：

1. **定边界、设计测试。** 在本计划对应功能处明确输入/输出、状态转换、资源与身份归属、兼容行为、非目标和验收标准；设计正常、边界、错误、重复、取消及并发/恢复等适用场景，区分 CPU 契约与真实 GPU/Harness/多机验收。重大取舍或冲突先由负责人确认。
2. **红。** 先写可执行的行为测试，在改动实现前运行，确认失败来自预期行为缺失或缺陷，并检查具体断言。依赖缺失、硬件不可用、收集错误或 fixture 故障不算有效的红；修复测试环境后再判断。
3. **绿。** 只实现约定范围内使测试通过的最小完整行为。测试替身限定在外部边界，不替换被测核心逻辑；不得靠放宽断言、跳过失败或缩小覆盖分母获得通过。
