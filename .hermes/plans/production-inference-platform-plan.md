# FreeChat 生产级小模型长上下文推理需求来源记录（已冻结）

> 状态：REFERENCE_ONLY / 禁止按本文件新增功能
> 适用分支：`main` 及其后续开发分支
> 权威关系：`.hermes/plans/agent-aware-inference-infra-plan.md` 是唯一上位实施契约。本文件中的需求只能按目标能力矩阵选择性采用，不得把聊天应用、Agent Control Plane 或工作流编排作为项目主线。
> 文档职责：保留需求来源和既有决策，供审计与冲突定位使用。
...
- 本文件不是新增实现的上位计划或最终验收清单。
- `.hermes/plans/context-management-plan.md` 作为上下文设计资料保留；发生冲突时，以 Agent-Aware Infrastructure 实施计划为准。
