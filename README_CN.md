...
1. **Attention Sink 布局**一致性最优：8K 上下文 83%，24K 上下文 62%。将关键信息放在首位位置利用了模型的注意力偏差。

2. **RAG 检索**在中低压缩率下持平或超过 Attention Sink。随上下文增长，检索的价值增大。

3. **Truncation 在长上下文下崩溃**：8K 时 50%，24K 时降到 25%。项目的压缩策略在同等条件下保持 60%+。

4. **LLM 话题提取不如关键词匹配**（0-17% vs 67%）。通用话题标签无法定位具体事实。
